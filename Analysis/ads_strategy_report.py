...
# Write to the user's renamed filename for convenience
REPORT_PATH = os.path.join(ANALYSIS_DIR, "google_ads_strategy_report_GPT5High.md")
...
def find_latest_csv_in_horizon(base_folder: str, horizon: str, pattern: str) -> str:
    """Look for CSV inside a horizon subfolder (e.g., '5 year', '2 year', '1 year').
    Fallback to base folder if not found."""
    if horizon:
        sub = os.path.join(base_folder, horizon)
        if os.path.isdir(sub):
            p = find_latest_csv(sub, pattern)
            if p:
                return p
    # Fallback to base
    return find_latest_csv(base_folder, pattern)


def load_timeline(theme: str, folder_path: str, horizon: str = "") -> pd.DataFrame:
    csv_path = find_latest_csv_in_horizon(folder_path, horizon, "multiTimeline*.csv")
    if not csv_path:
        return pd.DataFrame()
    try:
...
def load_geo(theme: str, folder_path: str, horizon: str = "") -> pd.DataFrame:
    csv_path = find_latest_csv_in_horizon(folder_path, horizon, "geoMap*.csv")
    if not csv_path:
        return pd.DataFrame()
    try:
        df = pd.read_csv(csv_path, skiprows=2)
...
def build_master_frames(timeline_horizon: str = "", geo_horizon: str = "") -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for item in os.listdir(PROJECT_ROOT):
        ...
        tdf = load_timeline(theme, dir_path, horizon=timeline_horizon)
        if not tdf.empty:
            time_rows.append(tdf)

        gdf = load_geo(theme, dir_path, horizon=geo_horizon)
        if not gdf.empty:
            geo_rows.append(gdf)
...
def write_report(master_time_5y: pd.DataFrame, master_geo_5y: pd.DataFrame,
                 master_time_2y: pd.DataFrame, master_geo_2y: pd.DataFrame,
                 master_time_1y: pd.DataFrame, master_geo_1y: pd.DataFrame) -> None:
    os.makedirs(ANALYSIS_DIR, exist_ok=True)

    # Section 2: Market Prioritization
    # Use 1-year for current volume, 5-year for CAGR
    avg_monthly = compute_avg_monthly_volume(master_time_1y if not master_time_1y.empty else master_time_5y)
    cagr = compute_cagr(master_time_5y)
    ...
    # Use 2-year for seasonality and geo overlap (recent pattern)
    season_vectors = build_seasonality_vectors(master_time_2y if not master_time_2y.empty else master_time_5y)
    geo_sets = compute_geo_sets(master_geo_2y if not master_geo_2y.empty else master_geo_5y)
    ...
    peaks = compute_peak_months(master_time_2y if not master_time_2y.empty else master_time_5y)
    # Use 2-year for top DMA per theme
    top_dma = top_dma_per_theme(master_geo_2y if not master_geo_2y.empty else master_geo_5y)
    all_themes_sorted = sorted(
        set((master_time_5y["theme"].unique() if not master_time_5y.empty else [])) |
        set((master_geo_5y["theme"].unique() if not master_geo_5y.empty else [])) |
        set((master_time_2y["theme"].unique() if not master_time_2y.empty else [])) |
        set((master_geo_2y["theme"].unique() if not master_geo_2y.empty else [])) |
        set((master_time_1y["theme"].unique() if not master_time_1y.empty else [])) |
        set((master_geo_1y["theme"].unique() if not master_geo_1y.empty else []))
    )
    ...
    # Use 2-year for DMA deep dive to reflect recent interest
    top5_dmas, dma_to_themes = top5_dmas_and_top3_themes(master_geo_2y if not master_geo_2y.empty else master_geo_5y)
    dma_blocks: List[str] = []
...
def main() -> None:
    # Build master frames for each horizon
    mt_5y, mg_5y = build_master_frames(timeline_horizon="5 year", geo_horizon="5 year")
    mt_2y, mg_2y = build_master_frames(timeline_horizon="2 year", geo_horizon="2 year")
    mt_1y, mg_1y = build_master_frames(timeline_horizon="1 year", geo_horizon="1 year")

    write_report(mt_5y, mg_5y, mt_2y, mg_2y, mt_1y, mg_1y)
